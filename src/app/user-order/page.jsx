...
export default function UserOrderPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 md:p-6 min-h-screen bg-gray-50">
      <h1 className="text-2xl font-black mb-6 text-gray-800 uppercase italic tracking-tighter">My Orders</h1>

      <div className="space-y-6">
        {orders.length > 0 ? (
          orders.map((order, index) => (
            <div key={order.id} className="border rounded-2xl p-5 shadow-sm bg-white overflow-hidden border-gray-100">
              ...
              <div className="flex flex-wrap justify-between items-center border-b pb-4 mb-4 gap-4">
                ...
                <div className="flex items-center gap-6">
                  ...
                  <span className={`px-4 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${
                    ...
                    order.status.toLowerCase() === 'cancelled' ? 'bg-red-100 text-red-700' : 'bg-black text-white'
                  }`}>
                    {order.status}
                  </span>
                </div>
              ...
              <div className="divide-y divide-gray-50">
                {order.order_items?.map((item) => (
                  <div key={item.id} className="flex gap-5 py-4 items-center">
                    <div className="h-24 w-20 bg-gray-50 rounded-xl border flex-shrink-0 overflow-hidden">
                      {item.image ? (
                        <img 
                          src={item.image} 
                      ...
                      <div className="flex justify-between items-start">
                        <div>
                          ...
                          <p className="text-[11px] text-gray-500 font-bold uppercase">
                            ...
                            Size: <span className="text-black">{item.size || 'N/A'}</span> | 
                            Color: <span className="text-black">{item.color || 'N/A'}</span>
                          </p>
                        </div>
